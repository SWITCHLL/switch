...
import { approveKycApplication, rejectKycApplication } from '../actions'
...
export function KycReviewActions({ applicationId, currentStatus }: KycReviewActionsProps) {
  ...
  if (currentStatus === 'APPROVED') {
    return (
      <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-500/10 px-3 py-1 text-[12px] font-semibold text-emerald-500">
  ...
  if (currentStatus === 'REJECTED') {
    return (
      <span className="inline-flex items-center gap-1.5 rounded-full bg-red-500/10 px-3 py-1 text-[12px] font-semibold text-red-500">
  ...
  function handleApprove() {
    ...
    startTransition(async () => {
      const result = await approveKycApplication(applicationId)
  ...
  function handleReject(e: React.FormEvent) {
    ...
    setError('')
    startTransition(async () => {
      const result = await rejectKycApplication(applicationId, note)
      if (!result.success) setError(result.error)
      else setMode('idle')
    })
  }
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <button
        ...
        className="border-border text-muted-foreground hover:text-foreground flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-[12px] font-medium transition-colors disabled:opacity-60"
      >
