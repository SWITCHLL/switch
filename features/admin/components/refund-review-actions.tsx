...
import { approveRefund, rejectRefund } from '@/features/payments/actions'
import { markRefundUnderReview } from '../actions'

interface RefundReviewActionsProps {
...
const TERMINAL: Record<string, { label: string; cls: string }> = {
  APPROVED: { label: 'Refunded', cls: 'bg-emerald-500/10 text-emerald-400' },
  REJECTED: { label: 'Rejected', cls: 'bg-red-500/10 text-red-500' },
}

export function RefundReviewActions({ refundRequestId, currentStatus }: RefundReviewActionsProps) {
  ...
  function handleMarkReview() {
    ...
    startTransition(async () => {
      const result = await markRefundUnderReview(refundRequestId)
  ...
  function handleApprove() {
    setError('')
    startTransition(async () => {
      const result = await approveRefund(refundRequestId)
      if (!result.success) setError(result.error)
    })
  }
  ...
  function handleReject(e: React.FormEvent) {
    ...
    setError('')
    startTransition(async () => {
      const result = await rejectRefund(refundRequestId, note)
      if (!result.success) setError(result.error)
      else setMode('idle')
    })
  }
