...
import {
  ShieldCheck, AlertCircle, Loader2, Tag, X,
  CheckCircle2, Ticket, CalendarDays,
} from 'lucide-react'
...
interface ShowCheckoutClientProps {
  event: {
    id:   string
    slug: string
    title: string
...
export function ShowCheckoutClient({ event, selections, subtotal }: ShowCheckoutClientProps) {
  ...
  async function handleApplyPromo() {
    ...
    const result = await validatePromoCode({
      ...
      eventId:       event.id,
      ticketTypeIds: [...new Set(selections.map((s) => s.ticketTypeId))],
      subtotal,
    })
  ...
  async function handlePay() {
    ...
    try {
      const res = await fetch('/api/payments/initialize-shows', {
        method:  'POST',
      ...
      if (json.free && json.reservationId) {
        router.push(
          `/events/${event.slug}/checkout/success?reservation=${json.reservationId}&type=shows`
        )
        return
      }
            ...
            {appliedPromo ? (
              <div className="flex items-center justify-between rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-3">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <p className="text-[13px] font-semibold text-emerald-500">{appliedPromo.code}</p>
                ...
                <button
                  ...
                  className="text-muted-foreground hover:text-foreground ml-2 rounded-lg p-1"
                >
                  <X className="h-4 w-4" />
                </button>
              </div>
...
function ShowOrderSummary({
  ...
  selections:   ShowSelection[]
  subtotal:     number
  appliedPromo: PromoValidation | null | undefined
}) {
  const effectiveTotal = appliedPromo ? appliedPromo.finalTotal : subtotal

  // Group by show for cleaner display
  const bySlot = new Map<string, ShowSelection[]>()
  for (const sel of selections) {
    const arr = bySlot.get(sel.timeSlotId) ?? []
    arr.push(sel)
    bySlot.set(sel.timeSlotId, arr)
  }
  ...
  return (
    <div className="border-border bg-surface rounded-2xl border p-5">
      ...
      <div className="space-y-4">
        {Array.from(bySlot.entries()).map(([slotId, entries]) => (
          <div key={slotId}>
            <div className="mb-1.5 flex items-center gap-1.5">
              <CalendarDays className="text-muted-foreground h-3.5 w-3.5 shrink-0" />
              <p className="text-[12.5px] font-semibold">{entries[0]?.slotLabel}</p>
            </div>
            <div className="space-y-1.5 pl-5">
              {entries.map((sel) => (
                <div key={sel.ticketTypeId} className="flex items-center justify-between text-[13px]">
                  <div>
                    <p className="font-medium">{sel.ticketTypeName}</p>
                    <p className="text-muted-foreground text-[11.5px]">
                      {sel.quantity} × {sel.price === 0 ? 'Free' : formatPrice(sel.price, sel.currency)}
                    </p>
                  </div>
                  <span className="font-medium tabular-nums">
                    {sel.price === 0 ? 'Free' : formatPrice(sel.price * sel.quantity, sel.currency)}
                  </span>
                </div>
              ))}
            </div>
